...
The preprocessing pipeline expects your local dataset directory to be named `dataset` (or targeted via your `RAW_ROOT` path variable) with internal class folders labeled numerically from `1` to `6`. The project should be structured as follows:

```text
├── README.md
├── main_pipeline.ipynb          # Jupyter Notebook containing code cells 1-13
└── dataset/                     # Root data directory (RAW_ROOT)
    ├── 1/                       # Folder containing Standard Sitting trials
    │   ├── Sub_1_Trial_1.csv
    │   └── Sub_1_Trial_2.csv
    ├── 2/                       # Folder containing Slouching trials
    ├── 3/                       # Folder containing Forward Bending trials
    ├── 4/                       # Folder containing Right Twisting trials
    ├── 5/                       # Folder containing Left Twisting trials
    └── 6/                       # Folder containing Standing trials
...
Below is the definitive biological and structural mapping of what each numerical directory contains:

| Folder Name | Internal Class ID | Posture Label | Biomechanical Context & Description |
| --- | --- | --- | --- |
| **`1`** | `0` | `Standard Sitting` | Neutral upright seated position; represents baseline spinal load. |
| **`2`** | `1` | `Slouching` | Thoracic and lumbar flexion; sustained forward kyphotic spine drop. |
| **`3`** | `2` | `Forward Bending` | Deep trunk flexion initiated from the hips; intense sagittal spinal displacement. |
...
| **`6`** | `5` | `Standing` | Upright extension; pelvis neutral; mass center balanced over base of support. |
